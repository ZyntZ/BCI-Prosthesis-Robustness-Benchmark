...
The paired decoder comparison is regenerated by `make compare-physionet-pipelines`. For regional dropout, named CSP rows with the same removed-channel fraction are averaged before matching to legacy unlabeled Riemann-LR rows; the generated validation JSON records this harmonization and confirms that no values were imputed.
